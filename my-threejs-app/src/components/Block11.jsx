...
// Component Model dùng GLTFLoader
function Model({ inView, modelUrl }) {
...
export default function Block11() {
    ...
    useEffect(() => {
        const fetchModel = async () => {
            try {
                const model = await fetchModelById(7);  // Lấy model với id = 7 từ API
                setModelUrl(model.url);  // Cập nhật URL của model
            } catch (error) {
        ...
        <div className="block11">
            ...
            <div className="block11_middle">
                <div className="block11_description" ref={descRef}>
                    ...
                    <div className="block11_description_second" ref={descSecondRef}>
                        {lines.map((line, index) => (
                            <motion.p
                                key={index}
                                initial={{ opacity: 0, y: 100 }}
                ...
                <div className="block11-canvas-part" ref={middleRef}>
                    <Canvas camera={{ position: [0, 0, 5], fov: 50 }}>
                        <ambientLight intensity={5} />
                        <directionalLight position={[5, 5, 5]} intensity={1} />
                        {modelUrl && <Model inView={isMiddleInView} modelUrl={modelUrl} />}
                        <OrbitControls
                            maxPolarAngle={Math.PI / 2}
                            minPolarAngle={0}
                            maxAzimuthAngle={Math.PI / 2}
