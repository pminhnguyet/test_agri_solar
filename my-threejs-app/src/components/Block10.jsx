...
// Component Model dùng URL từ API
function Model({ isIdeal, idealUrl, wiltedUrl }) {
    const { scene } = useGLTF(isIdeal ? idealUrl : wiltedUrl);
    const [showPopup, setShowPopup] = useState(false);
...
export default function Block10() {
    ...
    const [wiltedUrl, setWiltedUrl] = useState("");
    const [showHumidityPopup, setShowHumidityPopup] = useState(false);
    const [showLightPopup, setShowLightPopup] = useState(false);
    ...
    useEffect(() => {
        async function loadModels() {
            try {
                const model5 = await fetchModelById(5); // Ideal flower
                const model4 = await fetchModelById(4); // Wilted flower
                setIdealUrl(model5.url);
                setWiltedUrl(model4.url);
            } catch (error) {
    ...
    const handleHumidityClick = () => {
        setShowHumidityPopup(true);
        setTimeout(() => setShowHumidityPopup(false), 5000);
    };

    const handleLightClick = () => {
        setShowLightPopup(true);
        setTimeout(() => setShowLightPopup(false), 8000);
    };
        ...
        <div className="block10">
            ...
            <div className="block10_middle">
                <div className="block10_button">
                    ...
                    {showHumidityPopup && (
                        <div className="popup">
                            <p>Độ ẩm luôn được duy trì ổn định nhờ vào hệ thống tưới tự động thông minh, hoạt động bền vững nhờ nguồn năng lượng xanh từ pin mặt trời.</p>
                        </div>
                    )}
                    {showLightPopup && (
                        <div className="popup">
                ...
                <div className="block10-canvas-part">
                    <Canvas>
                        ...
                        <Suspense fallback={null}>
                            {idealUrl && wiltedUrl && (
                                <Model isIdeal={isIdeal} idealUrl={idealUrl} wiltedUrl={wiltedUrl} />
                            )}
                        </Suspense>
                        <OrbitControls maxDistance={8} />
                    </Canvas>
